...
;var SuggestRestaurant=function(){};SuggestRestaurant.IsSaving=!1,SuggestRestaurant.UploadPictureSuccess=function(a,b,c){if(c){var d=JSON.parse(b);d.success?($("#restaurant-picture > img").attr("src",d.pictureUrl),$("#restaurant-picture").show(),$("#PictureId").val(d.pictureId),$("#MobileImageName").val(d.mobileImageName)):alert("error")}},SuggestRestaurant.Init=function(){$("#ddlProvinces").change(function(a){var b=$(a.target).val();if(b!="undefined"&&b!=null&&b!=""){var c=new Date;$.get("/Common/GetDistrictsByProvinceId?provinceId="+b+"&t="+c.getTime(),function(a){var b=a.data;$("#ddlDistricts").empty();for(var c=0;c<b.length;c++)$("#ddlDistricts").append('<option value="'+b[c].Id+'">'+b[c].Name+"</option>")})}}),$("#suggestRestaurantForm").submit(function(a){a.preventDefault();if($(this).valid()&&!SuggestRestaurant.IsSaving){SuggestRestaurant.IsSaving=!0,$("#restaurant-saving").show();var b=$(this).serialize();$.ajax({url:"/Restaurant/SuggestRestaurant",type:"POST",data:b,success:function(a){a.success?(setTimeout(function(){location.reload()},2e3),$("#suggest-success").show(),$("#suggest-fail").hide()):($("#suggest-fail").show(),$("#suggest-success").hide(),SuggestRestaurant.IsSaving=!1),$("#restaurant-saving").hide()},failure:function(a){alert(a)}})}}),$("#upload-restaurant-picture").uploadify({buttonClass:"upload-restaurant-picture",buttonText:"Upload",fileObjName:"postedFile",fileTypeExts:"*.gif; *.jpg; *.png; *.bmp",removeTimeout:1,swf:"/scripts/uploadify/uploadify.swf",uploader:"/Restaurant/UploadSuggestRestaurantPicture",multi:!1,onUploadSuccess:function(a,b,c){SuggestRestaurant.UploadPictureSuccess(a,b,c)},itemTemplate:'<div id="${fileID}" class="uploadify-queue-item-profile">\t\t\t\t\t<div class="cancel">\t\t\t\t\t\t<a href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')">X</a>\t\t\t\t\t</div>\t\t\t\t\t<span class="fileName">${fileName} (${fileSize})</span><span class="data"></span>\t\t\t\t\t<div class="uploadify-progress-profile">\t\t\t\t\t\t<div class="uploadify-progress-bar-profile"><!--Progress Bar--></div>\t\t\t\t\t</div>\t\t\t\t</div>'})},$(function(){SuggestRestaurant.Init()})
